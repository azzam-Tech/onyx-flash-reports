...
start_pos = content.find('{"id":"by_customer","title":"حسب العميل"')
end_pos = content.find('{"id":"by_salesman","title":"حسب المندوب"')

new_by_customer_report = """{"id":"by_customer","title":"حسب العميل","params":[DFROM,DTO,CST,REP],"sql":\"\"\"
    WITH sales_mst AS (
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + new_by_customer_report + content[end_pos:]
    with open(r"privet\onyx_reports\app.py", "w", encoding="utf-8") as f:
        f.write(content)
    print("Successfully patched by_customer report query in app.py!")
else:
