...
try:
    with get_pooled_conn() as conn:
        with conn.cursor() as cur:
            ...
            # Save file
            out_path = os.path.join(os.path.dirname(__file__), '..', 'sub_groups_003.xlsx')
            wb.save(out_path)
            print(f"File successfully saved to: {out_path}")
            print(f"Total Sub-Groups extracted: {len(rows)}")
