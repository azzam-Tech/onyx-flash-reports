...
# 1. Update the TABS array to use the correct icon and structure
old_global_vars = '''{
        "id": "global_vars",
        "title": " ",
...
if '"id": "global_vars"' in content:
    # We replace the whole block manually
    content = re.sub(r'\{\s*"id":\s*"global_vars"[\s\S]*?\]\s*\}', new_global_vars, content)
else:
    # Append it to TABS
    content = content.replace("]\nTABMAP", ",\n" + new_global_vars + "\n]\nTABMAP")


# 2. Add backend logic to load targets
if "def _load_targets_raw():" not in content:
    targets_backend = """
...
# 3. Update index() to fetch salesmen and targets
...
# 4. Inject template logic into PAGE
# We look for `<div class="cnt">عدد الصفوف: {{rows|length}}</div>` in PAGE
page_ui_block = """
     {% if rpt.id == 'targets_ui' %}
         <style>
...
print("Targets UI applied successfully!")
