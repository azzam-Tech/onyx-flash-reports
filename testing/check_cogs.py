...
with get_conn() as con:
    with con.cursor() as cur:
        query = """
        SELECT SUM(NVL(im.I_QTY,0) * NVL(im.STK_COST,0))
        ...
        """
        cur.execute(query)
        print(f"Total True COGS: {cur.fetchone()[0]}")
        
        query2 = """
        SELECT SUM(NVL(DR_AMT,0))
        FROM IAS20261.IAS_POST_DTL
        WHERE SUBSTR(A_CODE, 1, 5) = '31101'
          AND DOC_DATE >= TO_DATE('2026-01-01', 'YYYY-MM-DD')
          AND DOC_DATE <= TO_DATE('2026-06-30', 'YYYY-MM-DD')
        """
        cur.execute(query2)
        print(f"Total GL COGS (what's on the paper): {cur.fetchone()[0]}")
