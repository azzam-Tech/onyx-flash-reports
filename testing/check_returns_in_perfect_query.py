...
def check_returns_in_perfect():
    ...
    FROM all_bills
    WHERE doc_type_name LIKE '%مرتجع%'
    ...
    rows = cur.fetchall()
    print(f"Sample Return Invoices in Perfect Bills Query ({len(rows)} returned):")
    for r in rows:
        print(" ", r)
