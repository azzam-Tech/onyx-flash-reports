...
def main():
    try:
        ...
        # Fetch INACTIVE, INACTIVE_SALES and C_CLASS_VAT
        query = """
            SELECT C_CODE, C_A_NAME, C_TAX_CODE, COMM_REG_NO, BUILDING_NO, STREET, DSTRCT_NM, EXTERNAL_POST, ADD_NO, CITY_NO, 
                   ...
                   NVL(C_CLASS_VAT, 0) as C_CLASS_VAT
        ...
        for row in rows:
            c_code, c_name, tax, crn, bld, street, dist, post, add_no, city, inactive, inactive_sales, c_class_vat = row
            
            # Exclude personal customers (C_CLASS_VAT = 1)
            if c_class_vat == 1:
                continue
        ...
        with open(r'c:\Users\amarn\OneDrive\Desktop\dbOnyxOnAntigravity\testing\fake_customers_active_only.json', 'w', encoding='utf-8') as f:
            json.dump(active_fake_customers, f, ensure_ascii=False, indent=4)
            
        print(f"Total fake (excluding personal): {all_fake}")
        print(f"Suspended fake (excluding personal): {suspended_fake}")
        print(f"Active fake (excluding personal): {len(active_fake_customers)}")
