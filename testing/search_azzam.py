...
with get_conn() as con:
    with con.cursor() as cur:
        cur.execute("SELECT table_name FROM all_tables WHERE owner='IAS20261' AND (table_name LIKE '%EMP%' OR table_name LIKE '%LOAN%' OR table_name LIKE '%ADVANCE%')")
        tables = [r[0] for r in cur.fetchall()]
        print("Employee/Loan tables:", tables)
