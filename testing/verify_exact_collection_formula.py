...
for g in ["cc", "rep", "period"]:
    cols, rows = run_sales_collection_summary({}, {"year_val": "2026", "period_type": "monthly", "period_val": "6", "grp_by": g})
    tot_col = sum(float(r[4].replace(',', '')) for r in rows)
    print(f"June 2026 - grp_by '{g}': Total Collection = {tot_col:,.2f}")
