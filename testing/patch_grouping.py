...
new_sql = '''"id":"true_income_statement","title":"قائمة الدخل (الحقيقية)","params":[DFROM,DTO,REP],"sql":"""
        WITH gl_base AS (
          SELECT 
              A_CODE as acc_code,
        ...
        SELECT 
            ...
            TO_CHAR(NVL(SUM(d.op_dr + d.mv_dr),0),'FM999,999,990.00') AS "إجمالي مدين",
            TO_CHAR(NVL(SUM(d.op_cr + d.mv_cr),0),'FM999,999,990.00') AS "إجمالي دائن",
            TO_CHAR(NVL(SUM(d.op_cr + d.mv_cr),0) - NVL(SUM(d.op_dr + d.mv_dr),0), 'FM999,999,990.00') AS "الرصيد النهائي"
        FROM all_data d
