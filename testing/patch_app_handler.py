...
content = content.replace("def run_report(rpt, args):", handler_code)

# Now add dispatch in run_report
dispatch_target = '    elif rpt["id"] == "main_wh_movement":\n        cols, rows = run_main_wh_movement(rpt, args)'
dispatch_replace = dispatch_target + '\n    elif rpt["id"] == "sales_collection_summary":\n        cols, rows = run_sales_collection_summary(rpt, args)'

content = content.replace(dispatch_target, dispatch_replace)

# Now add JS sync script to PAGE before </body></html>
js_code = '''
  <script>
  function syncPeriodFilters() {
    const pTypeSelect = document.querySelector('select[name="period_type"]');
    const pValSelect = document.querySelector('select[name="period_val"]');
    if (!pTypeSelect || !pValSelect) return;

    function update() {
      const val = pTypeSelect.value;
      const currentVal = pValSelect.getAttribute('data-val') || pValSelect.value || "all";
      
      let opts = [];
      if (val === 'annual') {
        opts = [["all", "السنة كاملة"]];
      } else if (val === 'monthly') {
        opts = [
          ["all", "جميع الشهور"],
          ["1", "01 - يناير"], ["2", "02 - فبراير"], ["3", "03 - مارس"],
          ["4", "04 - إبريل"], ["5", "05 - مايو"], ["6", "06 - يونيو"],
        ...
        opts = [
          ["all", "جميع الأرباع"],
          ["1", "الربع الأول (Q1)"], ["2", "الربع الثاني (Q2)"],
          ["3", "الربع الثالث (Q3)"], ["4", "الربع الرابع (Q4)"]
        ];
      } else if (val === 'semi_annual') {
        opts = [
          ["all", "جميع الأنصاف"],
          ["1", "النصف الأول (H1)"], ["2", "النصف الثاني (H2)"]
        ];
      }
      
      pValSelect.innerHTML = '';
      opts.forEach(o => {
        const optEl = document.createElement('option');
        optEl.value = o[0];
        optEl.textContent = o[1];
        if (o[0] === currentVal) optEl.selected = true;
        pValSelect.appendChild(optEl);
      });
    }

    pTypeSelect.addEventListener('change', update);
    update();
  }
  window.addEventListener('DOMContentLoaded', syncPeriodFilters);
  </script>
 </body></html>"""
'''

content = content.replace(" </body></html>\"\"\"", js_code)
