...
start_pos = content.find('{"id":"by_salesman","title":"حسب المندوب"')
end_pos = content.find('{"id":"net_sales_cc","title":"صافي المبيعات مع الخصومات (مراكز التكلفة)"')

new_by_salesman_report = """{"id":"by_salesman","title":"حسب المندوب","params":[DFROM,DTO,REP],"sql":\"\"\"
    WITH sales_mst AS (
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + new_by_salesman_report + content[end_pos:]
    with open(r"privet\onyx_reports\app.py", "w", encoding="utf-8") as f:
        f.write(content)
    print("Successfully patched by_salesman report query in app.py!")
else:
