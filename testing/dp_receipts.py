...
cur.execute("""
    SELECT p.DOC_NO, NVL(p.CR_AMT,0)
...
items = {str(doc): amt for doc, amt in receipts}
...
res, closest_val = subset_sum_dp(items, target)
if type(res) is list and len(res) > 0 and abs(closest_val - target) < 1.0:
    print(f"Exact match DOC_NOs: {res} (Sum: {closest_val})")
else:
