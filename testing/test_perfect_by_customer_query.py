...
def test_perfect_by_customer():
    ...
    params = {"date_from": date_from, "date_to": date_to, "c_code": None, "rep_code": None}
    cur.execute(sql, params)
    rows = cur.fetchall()
    cols = [d[0] for d in cur.description]

    print(f"Perfect By Customer Query returned {len(rows)} rows with {len(cols)} columns:")
    print("Cols:", cols)
    for r in rows[:10]:
        print(" ", r)
