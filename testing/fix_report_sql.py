...
def run_sales_collection_summary(rpt, args):
    ...
    if grp_by == "rep":
        ...
        if period_type == "quarterly":
            ...
            grp_col_p = "'Q' || TO_CHAR(p.DOC_DATE, 'Q')"
            ...
            grp_col_p = "CASE WHEN TO_CHAR(p.DOC_DATE, 'MM') <= '06' THEN 'النصف الأول (H1)' ELSE 'النصف الثاني (H2)' END"
            ...
            grp_col_p = "TO_CHAR(p.DOC_DATE, 'YYYY-MM')"
    ...
    col_trans AS (
      ...
      WHERE NVL(DOC_POST,0)=1 AND DOC_TYPE=1 AND JV_TYPE=2 AND NVL(CR_AMT,0)>0 AND C_CODE IS NOT NULL
        AND DOC_DATE >= TO_DATE(:date_from,'YYYY-MM-DD') AND DOC_DATE < TO_DATE(:date_to,'YYYY-MM-DD')+1
      UNION ALL
      SELECT {grp_sales_b}, 0, 0, NVL(p.DR_AMT,0) - NVL(b.DISC_AMT,0), 0, 0, 0, 0
