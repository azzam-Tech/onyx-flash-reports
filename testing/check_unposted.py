...
with get_conn() as con:
    with con.cursor() as cur:
        ...
        cur.execute("""
            SELECT DOC_NO, TO_CHAR(DOC_DATE, 'YYYY-MM-DD'), TO_CHAR(UP_DATE, 'YYYY-MM-DD'), CR_AMT, C_CODE
            ...
            WHERE NVL(DOC_POST,0)=1 AND DOC_TYPE=2 AND NVL(CR_AMT,0)>0 AND C_CODE IS NOT NULL
              ...
              AND UP_DATE >= TO_DATE('2026-07-19', 'YYYY-MM-DD')
        """)
        print("--- Updated Receipts ---")
        for row in cur.fetchall():
            print(f"DOC_NO: {row[0]}, DOC_DATE: {row[1]}, UP_DATE: {row[2]}, AMT: {row[3]}, C_CODE: {row[4]}")
