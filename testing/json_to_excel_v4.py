...
def is_fake(val):
    if not val:
        return True
    val = str(val).strip()
    # List of known exact fake entries
    ...
    if len(set(val)) == 1 and val[0].isdigit():
        return True
    # Removed the aggressive 'if "12345" in val' check as it flags valid ZATCA numbers!
    return False
...
def main():
    try:
        ...
        csv_path = r'c:\Users\amarn\OneDrive\Desktop\dbOnyxOnAntigravity\Active_Fake_Customers_V4.csv'
