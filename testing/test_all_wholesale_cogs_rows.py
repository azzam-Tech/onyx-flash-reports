...
with get_conn() as con:
    with con.cursor() as cur:
        cur.execute(sql_wholesale_full, binds)
        cols = [d[0] for d in cur.description]
        rows = cur.fetchall()
        print(f"WHOLESALE COGS REPORT SUCCESS: Fetched {len(rows)} rows, {len(cols)} columns.")
        for r in rows:
            if r[0] in ('311010001', '311030001', '311060001'):
                print(r)
