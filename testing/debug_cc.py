...
sql_cc = """
    ...
    ext_disc_base AS (
        SELECT TO_CHAR(p.CC_CODE) as grp_code, SUM(NVL(p.CR_AMT,0)) as ext_disc_with_vat
    ...
    col_trans AS (
      SELECT TO_CHAR(p.CC_CODE) as grp_code, p.CR_AMT as rcpt, 0 as net_jrn, 0 as cash_sales, 0 as cash_ret, 0 as unposted_rcpt
      FROM IAS20261.IAS_POST_DTL p
      ...
      UNION ALL
      SELECT TO_CHAR(p.CC_CODE), 0, 0, 0, p.CR_AMT, 0
      FROM IAS20261.IAS_POST_DTL p
...
with get_conn() as con:
    with con.cursor() as cur:
        cur.execute(sql_cc, {'date_from': date_from, 'date_to': date_to, 'rep_code': rep_code})
        row = cur.fetchone()
        print("SREEN CC LOGIC:")
