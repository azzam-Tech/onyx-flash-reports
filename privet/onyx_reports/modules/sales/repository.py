...
def get_bills_sql():
    ...
    WITH sales_invoices AS (
      ...
      FROM IAS_BILL_MST b
      LEFT JOIN CUSTOMER c ON c.C_CODE = b.C_CODE
      LEFT JOIN SALES_MAN sm ON TO_CHAR(sm.REPRS_CODE) = TO_CHAR(b.REP_CODE)
    ...
    return_invoices AS (
      ...
      LEFT JOIN CUSTOMER c ON c.C_CODE = r.C_CODE
...
def get_by_item_sql():
    ...
    item_sales AS (
        ...
        LEFT JOIN IAS_ITM_MST m ON TO_CHAR(m.I_CODE) = TO_CHAR(dt.I_CODE)
        WHERE b.BILL_DATE >= TO_DATE(:date_from,'YYYY-MM-DD')
          AND b.BILL_DATE < TO_DATE(:date_to,'YYYY-MM-DD')+1
          AND b.BILL_DOC_TYPE IN (1,4,8)
    ...
    item_returns AS (
        SELECT rdt.I_CODE as item_code,
               ...
               ELSE 0 END as extra_header_disc
        FROM IAS_RT_BILL_DTL rdt
        JOIN IAS_RT_BILL_MST r 
        ...
        LEFT JOIN IAS_ITM_MST m ON TO_CHAR(m.I_CODE) = TO_CHAR(rdt.I_CODE)
        WHERE r.RT_BILL_DATE >= TO_DATE(:date_from,'YYYY-MM-DD')
          AND r.RT_BILL_DATE < TO_DATE(:date_to,'YYYY-MM-DD')+1
          AND r.RT_BILL_DOC_TYPE IN (1,4,8)
    ...
    LEFT JOIN IAS_ITM_MST m ON TO_CHAR(m.I_CODE) = TO_CHAR(t.item_code)
...
def get_net_sales_cc_sql():
    return """
       ...
       LEFT JOIN COST_CENTERS cc ON cc.CC_CODE = NVL(NVL(s.CC_CODE, r.CC_CODE), d.CC_CODE)
