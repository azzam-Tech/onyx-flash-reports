...
def run_sales_collection_summary(rpt, args):
    ...
    if grp_by == 'rep':
        ...
        join_table = 'LEFT JOIN SALES_MAN sm ON TO_CHAR(sm.REPRS_CODE) = NVL(ns.grp_code, cs.grp_code)'
        ...
        join_table = 'LEFT JOIN CUSTOMER c ON TO_CHAR(c.C_CODE) = NVL(ns.grp_code, cs.grp_code)'
...
def run_debt_movement_summary(rpt, args):
    ...
    if grp_by == 'rep':
        ...
        grp_sales_b = 'TO_CHAR(b.REP_CODE)'
        grp_ret = 'TO_CHAR(REP_CODE)'
        join_table = 'LEFT JOIN SALES_MAN sm ON TO_CHAR(sm.REPRS_CODE) = ac.grp_code'
        name_expr = 'MAX(sm.REPRS_A_NAME)'
        code_label = 'كود المندوب'
        name_label = 'اسم المندوب'
    elif grp_by == 'customer':
        ...
        grp_ret = 'TO_CHAR(C_CODE)'
        join_table = 'LEFT JOIN CUSTOMER c ON TO_CHAR(c.C_CODE) = ac.grp_code'
        name_expr = 'MAX(c.C_A_NAME)'
        code_label = 'كود العميل'
        name_label = 'اسم العميل'
        rep_filter = 'AND (:rep_code IS NULL OR TO_CHAR(c.REP_CODE) = :rep_code)'
...
def handle_sales_report(report_id, rpt, args):
    ...
    if report_id == 'net_debt_movement_summary':
        return run_net_debt_movement_summary(rpt, args)
...
def run_net_debt_movement_summary(rpt, args):
    ...
    cols = [code_label, name_label, 'المديونية الافتتاحية', 'صافي المبيعات شامل الضريبة', 'إجمالي التحصيل', 'نسبة التحصيل', 'المديونية النهائية', 'إجمالي المبيعات بدون الضريبة', 'الهدف']
    rows = []
    with get_conn() as con:
        with con.cursor() as cur:
            params = {'date_from': date_from, 'date_to': date_to}
            ...
            for c_code, c_name, open_b, ns_vat, ns_no_vat, col, close_b in cur.fetchall():
                ...
                target_val = get_target_amount(year_val, period_type, period_val, grp_by, c_code)
                target_str = f'{target_val:,.2f}' if target_val > 0 else ''
                rows.append((c_code, c_name or str(c_code), f'{ob_val:,.2f}', f'{ns_vat_val:,.2f}', f'{col_val:,.2f}', f'{col_ratio:,.2f}%', f'{closing_val:,.2f}', f'{ns_no_vat_val:,.2f}', target_str))
    return (cols, rows)
