    ...
    return """
        ...
        inventory_mov AS (
            SELECT 
                ...
                SUM(CASE WHEN I_DATE >= TO_DATE(:date_from, 'YYYY-MM-DD') AND I_DATE < TO_DATE(:date_to, 'YYYY-MM-DD')+1 THEN 
                    CASE WHEN DOC_TYPE = 1 THEN NVL(dt.I_QTY,0) WHEN DOC_TYPE = 3 THEN -NVL(dt.I_QTY,0) ELSE 0 END 
                ELSE 0 END) as sales_qty,
                SUM(CASE WHEN I_DATE >= TO_DATE(:date_from, 'YYYY-MM-DD') AND I_DATE < TO_DATE(:date_to, 'YYYY-MM-DD')+1 THEN 
                    CASE WHEN DOC_TYPE = 2 THEN NVL(dt.I_QTY,0) WHEN DOC_TYPE = 4 THEN -NVL(dt.I_QTY,0) ELSE 0 END 
                ELSE 0 END) as pur_qty,
