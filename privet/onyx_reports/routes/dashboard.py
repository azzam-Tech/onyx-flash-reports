...
def compute_dash(f, t):
    ...
    try:
        sqls = {
            ...
            "netprofit": "SELECT NVL(SUM(NVL(p.CR_AMT,0)-NVL(p.DR_AMT,0)),0) FROM IAS_POST_DTL p JOIN ACCOUNT a ON a.A_CODE=p.A_CODE WHERE (1=1) AND a.A_REPORT=2 AND p.DOC_DATE>="+P+" AND p.DOC_DATE<"+Q,
        ...
        try:
            with get_pooled_conn() as con:
                with con.cursor() as cur:
                    ...
                    cur.execute("SELECT TO_CHAR(V_CODE), SUM(NVL(CR_AMT,0) - NVL(DR_AMT,0)) FROM IAS_POST_DTL WHERE (1=1) AND V_CODE IS NOT NULL AND DOC_DATE < TO_DATE(:dt, 'YYYY-MM-DD')+1 GROUP BY TO_CHAR(V_CODE)", {'dt': t})
                    ...
                    ext_disc_base = f"SELECT ROUND(SUM(NVL(CR_AMT,0)) / 1.15, 2) FROM IAS_POST_DTL WHERE DOC_TYPE=15 AND NVL(CR_AMT,0)>0 AND (1=1) AND DOC_DATE >= {P} AND DOC_DATE < {Q}"
                    ...
                    SELECT SUM(rcpt) FROM (
                      SELECT SUM(CR_AMT) as rcpt FROM IAS_POST_DTL WHERE (1=1) AND DOC_TYPE=2 AND NVL(CR_AMT,0)>0 AND C_CODE IS NOT NULL AND DOC_DATE >= {P} AND DOC_DATE < {Q}
                      UNION ALL
                      SELECT SUM(CR_AMT) FROM IAS_POST_DTL WHERE (1=1) AND DOC_TYPE=1 AND JV_TYPE=2 AND NVL(CR_AMT,0)>0 AND C_CODE IS NOT NULL AND DOC_DATE >= {P} AND DOC_DATE < {Q}
                      UNION ALL
                      SELECT SUM(NVL(p.DR_AMT,0)) FROM IAS_BILL_MST b JOIN IAS_POST_DTL p ON p.DOC_NO=b.BILL_NO AND p.DOC_SER=b.BILL_SER AND p.DOC_TYPE=4 AND TO_CHAR(p.A_CODE) LIKE '111%' WHERE b.BILL_DOC_TYPE=1 AND (1=1) AND p.DR_AMT>0 AND b.BILL_DATE >= {P} AND b.BILL_DATE < {Q}
                      UNION ALL
                      SELECT SUM(-CR_AMT) FROM IAS_POST_DTL WHERE (1=1) AND DOC_TYPE=5 AND A_CODE LIKE '111%' AND NVL(CR_AMT,0)>0 AND DOC_DATE >= {P} AND DOC_DATE < {Q}
                    )
                    """
                    ...
                    me_q = f"SELECT TO_CHAR(DOC_DATE, 'YYYY-MM'), ROUND(SUM(NVL(CR_AMT,0)) / 1.15, 2) FROM IAS_POST_DTL WHERE DOC_TYPE=15 AND NVL(CR_AMT,0)>0 AND (1=1) AND DOC_DATE >= {P} AND DOC_DATE < {Q} GROUP BY TO_CHAR(DOC_DATE, 'YYYY-MM')"
                    ...
                    SELECT mo, SUM(rcpt) FROM (
                      SELECT TO_CHAR(DOC_DATE, 'YYYY-MM') as mo, SUM(CR_AMT) as rcpt FROM IAS_POST_DTL WHERE (1=1) AND DOC_TYPE=2 AND NVL(CR_AMT,0)>0 AND C_CODE IS NOT NULL AND DOC_DATE >= {P} AND DOC_DATE < {Q} GROUP BY TO_CHAR(DOC_DATE, 'YYYY-MM')
                      UNION ALL
                      SELECT TO_CHAR(DOC_DATE, 'YYYY-MM'), SUM(CR_AMT) FROM IAS_POST_DTL WHERE (1=1) AND DOC_TYPE=1 AND JV_TYPE=2 AND NVL(CR_AMT,0)>0 AND C_CODE IS NOT NULL AND DOC_DATE >= {P} AND DOC_DATE < {Q} GROUP BY TO_CHAR(DOC_DATE, 'YYYY-MM')
                      UNION ALL
                      SELECT TO_CHAR(b.BILL_DATE, 'YYYY-MM'), SUM(NVL(p.DR_AMT,0)) FROM IAS_BILL_MST b JOIN IAS_POST_DTL p ON p.DOC_NO=b.BILL_NO AND p.DOC_SER=b.BILL_SER AND p.DOC_TYPE=4 AND TO_CHAR(p.A_CODE) LIKE '111%' WHERE b.BILL_DOC_TYPE=1 AND (1=1) AND p.DR_AMT>0 AND b.BILL_DATE >= {P} AND b.BILL_DATE < {Q} GROUP BY TO_CHAR(b.BILL_DATE, 'YYYY-MM')
                      UNION ALL
                      SELECT TO_CHAR(DOC_DATE, 'YYYY-MM'), SUM(-CR_AMT) FROM IAS_POST_DTL WHERE (1=1) AND DOC_TYPE=5 AND A_CODE LIKE '111%' AND NVL(CR_AMT,0)>0 AND DOC_DATE >= {P} AND DOC_DATE < {Q} GROUP BY TO_CHAR(DOC_DATE, 'YYYY-MM')
                    ) GROUP BY mo
                    """
                    mc_dict = {}
